...
def travoltas_busiest_years
  ...
  execute(<<-SQL)
    ...
    JOIN
      actors ON castings.actor_id = actors.id
    GROUP BY
      movies.yr, actors.name
    HAVING
      actors.name = 'John Travolta' AND COUNT(movies.title) >= 2
  SQL
end
...
def andrews_films_and_leads
  ...
  execute(<<-SQL)
    ...
    FROM
      movies
    JOIN
      castings ON castings.movie_id = movies.id AND ord = 1
    JOIN
      actors ON castings.actor_id = actors.id
    WHERE
      movies.title IN (
...
def prolific_actors
  ...
  execute(<<-SQL)
    ...
    JOIN
      castings ON castings.actor_id = actors.id
    GROUP BY
      actors.id, castings.actor_id, castings.ord
    HAVING
      actors.id = castings.actor_id AND COUNT(castings.ord) >= 15 AND castings.ord = '1'
    ORDER BY
      actors.name
  SQL
end
